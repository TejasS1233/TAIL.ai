...
export default function Login() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      const message = err?.response?.data?.message?.toString?.() || err?.message || "Google login failed.";
      toast.error(message);
    ...
    <div className="min-h-screen flex items-center justify-center bg-background px-4">
      <Card className="max-w-sm w-full">
        ...
        <CardContent className="space-y-6">
          ...
          <Button className="w-full gap-3" variant="secondary" onClick={handleGoogleLogin} disabled={googleLoading}>
            <GoogleLogo />
